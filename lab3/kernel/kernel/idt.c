...
void initIdt() {
	...
	setIntr(idt + 0x80, SEG_KCODE, (uint32_t)irqSyscall, DPL_USER); // for int 0x80, interrupt vector is 0x80, 
		
	setIntr(idt + 0x20, SEG_KCODE, (uint32_t)irqTimer, DPL_KERN);															//Interruption is disabled
	/* 写入IDT */
